...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
      ...
      .then(data => {
        ...
        if (product) {
          setFormData({
            ...
            color: product.color || '',
            brand: product.brand || '',
            thriftStory: product.thriftStory || '',
            isFreshDrop: product.isFreshDrop || false
          });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedFiles.length === 0 && existingImages.length === 0) {
      showToast('Please have at least one image', 'error');
      return;
    }
    ...
    try {
      ...
      const finalImages = [...existingImages, ...formattedImages];
  ...
  if (loading) return <div className="container py-12 text-center text-white">Loading product details...</div>;
  ...
  return (
    <div className="container py-12 max-w-4xl mx-auto animate-fade-in">
      ...
      <div className="card p-8 md:p-10 shadow-xl border border-[var(--border-color)] bg-gradient-to-br from-[var(--surface-color)] to-[var(--bg-color)]">
        <h1 className="text-3xl font-extrabold mb-8 text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-400">Edit Product</h1>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-8">
          ...
          <div className="flex flex-col gap-3">
            ...
            {existingImages.length > 0 && (
              <div className="mb-4">
                <p className="text-xs text-secondary mb-2">Current Images (Click X to remove)</p>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {existingImages.map((bgStr, i) => {
                    const bgUrl = bgStr.replace("url('", "").replace("') center/cover", "");
                    return (
                      <div key={i} className="relative aspect-square rounded-xl overflow-hidden border border-[var(--border-color)] group shadow-sm">
                        <img src={bgUrl} alt="existing" className="w-full h-full object-cover" />
                        <button 
                          type="button" 
            ...
            >
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-secondary mt-1">Add new images</p>
              </div>
              <input 
                ref={fileInputRef}
                type="file" 
            ...
            {previewUrls.length > 0 && (
              <div className="mt-4">
                <p className="text-xs text-secondary mb-2">New Images to Upload</p>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col gap-2">
              ...
              <select required name="size" value={formData.size} onChange={handleChange} className="p-4 rounded-xl border border-[var(--border-color)] bg-[var(--surface-color)] focus:border-[var(--accent-color)] text-white outline-none transition-colors appearance-none cursor-pointer">
                ...
                <option value="US 9 / EU 42.5">US 9 / EU 42.5</option>
                <option value="US 10 / EU 44">US 10 / EU 44</option>
                <option value="US 11 / EU 45">US 11 / EU 45</option>
              </select>
            </div>
            
            <div className="flex flex-col gap-2">
              <label className="text-sm font-semibold text-secondary uppercase tracking-wider">Color *</label>
              <select required name="color" value={formData.color} onChange={handleChange} className="p-4 rounded-xl border border-[var(--border-color)] bg-[var(--surface-color)] focus:border-[var(--accent-color)] text-white outline-none transition-colors appearance-none cursor-pointer">
          ...
          <button type="submit" disabled={loading} className="btn-primary mt-6 p-5 text-lg font-bold rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all disabled:opacity-50 disabled:hover:translate-y-0 disabled:hover:shadow-lg flex justify-center items-center gap-2">
            {uploading ? (
              <span className="flex items-center gap-2"><UploadCloud className="animate-bounce" /> Updating Images...</span>
            ) : loading ? (
              'Saving...'
            ) : (
              'Save Changes'
